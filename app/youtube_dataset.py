...
YOUTUBE_AUDIO_DIRPATH = os.path.join(YOUTUBE_DIRPATH, "audio") # todo: deprecate, get from the dataset instead (so we can use a different path in colab)
...
class YoutubeDataset:
    ...
    def __init__(self, dirpath=YOUTUBE_DIRPATH):
        self.dirpath = dirpath
        self.audio_dirpath = os.path.join(self.dirpath, "audio")
        self.audio_splits_dirpath = os.path.join(self.dirpath, "audio_splits")


    @property
    def artist_names(self):
        return [fname for fname in os.listdir(self.audio_dirpath) if fname not in [".DS_Store"]]

    @cached_property
    def audio_files(self):
        files = []
        for artist_name in self.artist_names:
            artist_dirpath = os.path.join(self.audio_dirpath, artist_name)
    ...
    def features_csv_filepath(self, track_length, n_mfcc):
        features_dirpath = os.path.join(self.dirpath, "features")
